...
class ResNet(nn.Module):
    def __init__(self, conv_layer, linear_layer, block, num_blocks, num_classes=10, contrastive_learning=True):
...
def resnet18(conv_layer, linear_layer, init_type, contrastive_learning, **kwargs ):
    assert init_type == "kaiming_normal", "only supporting default init for Resnets"
    return ResNet(conv_layer, linear_layer, BasicBlock, [2, 2, 2, 2], contrastive_learning, **kwargs)
...
def test(contrastive_learning):
    net = resnet18(nn.Conv2d, nn.Linear, "kaiming_normal", contrastive_learning)
